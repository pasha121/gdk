...
namespace sdk {
    namespace {
        ...
        static void create_ga_transaction_impl(session_impl& session, nlohmann::json& result)
        {
            ...
            auto create_tx_outputs = [&](const std::string& asset_id) {
                ...
                if (!have_change_addr) {
                    ...
                    if (is_liquid) {
                        if (is_electrum) {
                            // FIXME: ga_rust.get_receive_address() should return
                            // unblinded addrs, and have the auth handler blind them,
                            // so this code can be removed.
                            unblind_address(net_params, change_address);
                        }
        ...
        {
            const auto& net_params = session.get_network_parameters();
            const auto txid = get_script_hash(net_params, u, tx, index);
            const std::string private_key = json_get_value(u, "private_key");

            if (!private_key.empty()) {
                const auto private_key_bytes = h2b(private_key);
                const auto user_sig = ec_sig_from_bytes(private_key_bytes, txid);
                ...
                const auto user_sig = signer->sign_hash(path, txid);
                const auto der = ec_sig_to_der(user_sig, true);
    ...
    void unblind_address(const network_parameters& net_params, nlohmann::json& addr)
    {
        ...
        if (boost::starts_with(address.get<std::string>(), blech32_prefix)) {
            auto&& fn = confidential_addr_to_addr_segwit;
            address = fn(address, blech32_prefix, net_params.bech32_prefix());
        } else {
